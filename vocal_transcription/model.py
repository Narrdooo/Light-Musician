...
class VocalNet(nn.Module):
    def __init__(self, frames_per_second, classes_num):
        ...
        self.logmel_extractor = LogmelFilterBank(sr=sample_rate, 
            n_fft=window_size, n_mels=self.mel_bins, fmin=fmin, fmax=fmax, ref=ref, 
            amin=amin, top_db=top_db, freeze_parameters=True)

        self.bn0 = nn.BatchNorm2d(self.mel_bins, momentum)
    ...
    def forward(self, input):
        ...
        Outputs:
          output_dict: dict, {
            ...
            'instrument_output': (batch_size, time_steps, classes_num)
          }
        """

        spe = self.spectrogram_extractor(input)   # (batch_size, 1, time_steps, freq_bins)
        mel = self.logmel_extractor(spe)    # (batch_size, 1, time_steps, mel_bins)
        trans = transforms.Resize((1001, self.mel_bins))
        mel = trans(mel)
        x = mel.transpose(1, 3)
        x = self.bn0(x)
        x = x.transpose(1, 3)
        ...
        x = torch.cat((reg_onset_output, (reg_onset_output ** 0.5) * frame_output.detach()), dim=2)
