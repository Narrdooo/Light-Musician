...
class MusicnetDataset(object):
    ...
    def __getitem__(self, meta):
        audio_path, label_path, bgn_time = meta
        ...
        target_dict = self.label_process(label_path, bgn_sample, end_sample)
    ...
    def label_process(self, label_path, bgn_sample, end_sample):
        """
        return: 
            target_dict:{
                ...
                'instrument_roll': (frames_num, n_notes), 
        ...
        instrument_roll = np.zeros((frames_num, self.n_notes))
        mask_roll = np.ones((frames_num, self.n_notes))

        label_dataframe = pd.read_csv(label_path)
        # label_dataframe = label_dataframe[label_dataframe['end_time'] > bgn_sample]
        # label_dataframe = label_dataframe[label_dataframe['start_time'] < end_sample]

        for _, row in label_dataframe.iterrows():
            if row['end_time'] <= bgn_sample or row['start_time'] >= end_sample:
                continue

            note = min(87, int(row['note']))
            bgn_frame = int((row['start_time'] - bgn_sample) / self.convert_ratio)
            end_frame = int((row['end_time'] - bgn_sample) / self.convert_ratio)
            
            frame_roll[max(bgn_frame, 0): min(end_frame+1, frames_num), note] = 1
            instrument_roll[bgn_frame: end_frame+1, note] = int(row['instrument'])

            if bgn_frame >= 0:              
                onset_roll[bgn_frame, note] = 1
                reg_onset_roll[bgn_frame, note] = \
                    ((row['start_time'] - bgn_sample) / self.sample_rate) - (bgn_frame / self.frames_per_second)
            else:
            ...
            if end_frame < frames_num:      
                ...
                reg_offset_roll[end_frame, note] = \
                    ((row['end_time'] - bgn_sample) / self.sample_rate) - (end_frame / self.frames_per_second)
            else:
...
class Sampler(object):
    def __init__(self, data_dir, label_dir, segment_seconds, hop_seconds, sample_rate, 
        ...
        musicnet_df = pd.read_csv('data/musicnet/musicnet_metadata.csv', index_col=0)

        self.segment_list = []
        for data in os.listdir(self.data_dir):
            filename = data.rstrip('.wav')
            audio_path = os.path.join(self.data_dir, "%s.wav"%filename)
            label_path = os.path.join(self.label_dir, "%s.csv"%filename)
            bgn_time = 0
            len_time = musicnet_df.loc[int(filename), 'seconds']
            while(bgn_time + self.segment_seconds < len_time):
                self.segment_list.append([audio_path, label_path, bgn_time])
...
class TestSampler(Sampler):
    def __init__(self, data_dir, label_dir, segment_seconds, hop_seconds, sample_rate, 
            batch_size, type, random_seed=1234):
        super().__init__(data_dir, label_dir, segment_seconds, hop_seconds, sample_rate, 
